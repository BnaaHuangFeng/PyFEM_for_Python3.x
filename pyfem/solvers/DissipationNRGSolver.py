  ...
  def run(self, props, globdat):
    ...
    if globdat.cycle == 10:
      ...
      if self.method == 'force-controlled':
        ...
        h =  0.5 * lam0 * fhat
        w = -0.5 * np.dot((a-Da), fhat)
        g =  0.5 * np.dot((lam0 * Da - Dlam * (a[:] - Da[:])), fhat) - dtau

        d1 = globdat.dofs.solve(K, lam*fhat - fint)
        d2 = globdat.dofs.solve(K, -1.0*fhat)

        denom  = np.dot(h, d2) - w

        da     = d1 - (d2 * (np.dot(h, d1) + g)) / denom
        dlam   = -g - (np.dot(-1.0*h, d1) - g * (1.0 + denom)) / denom;
    ...
    globdat.dissnrg = 0.5 * np.dot((lam0 * Da - (lam-lam0) * (a - Da)), fhat)
