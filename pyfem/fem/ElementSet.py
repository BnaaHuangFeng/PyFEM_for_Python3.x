...
class ElementSet( itemList ):
  ...
  def add ( self, ID, modelName, elementNodes ):  
    ...
    module_name = 'pyfem.elements.' + modelType
    element_module = import_module(module_name)
    element_class = getattr(element_module, modelType)
    elem = element_class(elementNodes, modelProps)
    #Create the element
